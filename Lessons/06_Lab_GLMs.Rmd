...
Litter <- read.csv("~/R/EDA-Fall2022/Data/Processed/NEON_NIWO_Litter_mass_trap_Processed.csv", stringsAsFactors = TRUE)
# Set date to date format
...
EPA.Air.Cor <- 
  ...
  na.omit()
EPAirCor <- cor(EPA.Air.Cor)
corrplot(EPAirCor, method = "ellipse")
corrplot.mixed(EPAirCor, upper = "ellipse")
...
TPAIC <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + meanLng + meanLat)

step(TPAIC)
ACI_Model <- lm(data = EPAair, PM2.5 ~ Ozone + meanLat + Month)
summary(ACI_Model)
...
PM2.5Regression <- lm(EPAair$PM2.5 ~ EPAair$meanLat)
...
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  ...
  stat_summary(geom = "text", fun = max, vjust = -1, size = 2,
...
Litter.Totals.plot.New <- ggplot(Litter, aes(y = dryMass, x = plotID, color = functionalGroup)) +
  geom_boxplot()
print(Litter.Totals.plot.New)
```
