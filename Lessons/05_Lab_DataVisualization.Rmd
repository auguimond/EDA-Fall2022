...
Surface <- filter(PeterPaul.chem.nutrients, depth %in% c(0))
ggplot(Surface, aes(x=daynum, y=temperature_C)) + geom_point()
# Color your points by year, and facet by lake in two rows.
ggplot(Surface, aes(x=daynum, y=temperature_C, color=year4)) + geom_point()+
facet_wrap(vars(lakename), nrow=2)+ylab("Surface Tmperature")
# Change the ylab name


#2. 
# Plot temperature by date. Color your points by depth.
ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C, color=depth)) + geom_point()+
facet_wrap(vars(lakename), nrow=2)+ylab("Surface Tmperature")
